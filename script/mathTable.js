...
for (var i = 1; i <= 20; i++) {
  for (var j = 1; j <= 10; j++) {
    var equation = i + " × " + j + " = " + (i * j);
    var boxKey = "box" + ((i - 1) * 10 + j);
    equations[boxKey] = equation;
  }
}
...
for (var i = 0; i < boxes.length; i++) {
	boxes[i].addEventListener("click", function () {
		...
		*/
		...
		var modifiedText = text.replace("×", "into"); // Modify the text to replace the multiplication sign with "into"
...
var boxes = document.getElementsByClassName('table-container');
for (var i = 0; i < boxes.length; i++) {
